...
local Luna = { 
	Folder = "Luna", 
	Options = {}, 
	ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromHex("#0091FF")), ColorSequenceKeypoint.new(0.50, Color3.fromHex("#0091FF")), ColorSequenceKeypoint.new(1.00, Color3.fromHex("#0091FF"))} 
}
...
Luna.Theme = DarkTheme

local Animation = {
	Fast = TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
	Hover = TweenInfo.new(0.16, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
	Normal = TweenInfo.new(0.22, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
	Reveal = TweenInfo.new(0.30, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
	Dialog = TweenInfo.new(0.35, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
	Window = TweenInfo.new(0.42, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
	Slider = TweenInfo.new(0.10, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
	SliderSet = TweenInfo.new(0.20, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
}
...
local function Hide(Window, bind, notif)
	...
	for _, tabbtn in ipairs(Window.Navigation.Tabs:GetChildren()) do
		if tabbtn.ClassName == "Frame" and tabbtn.Name ~= "InActive Template" then
			TweenService:Create(tabbtn, Animation.Normal, {BackgroundTransparency = 1}):Play()
			TweenService:Create(tabbtn.ImageLabel, Animation.Normal, {ImageTransparency = 1}):Play()
			TweenService:Create(tabbtn.DropShadowHolder.DropShadow, Animation.Normal, {ImageTransparency = 1}):Play()
			TweenService:Create(tabbtn.UIStroke, Animation.Normal, {Transparency = 1}):Play()
		end
	end
...
local function SolidSequence(color)
...
local function HasAncestorNamed(instance, fragment)
	fragment = string.lower(fragment)
	local current = instance
	while current and current ~= LunaUI do
		if string.find(string.lower(current.Name), fragment, 1, true) then
			return true
		end
...
local function ApplyDarkTheme(root)
	for _, instance in ipairs(root:GetDescendants()) do
		local name = string.lower(instance.Name)
		local inColorPicker = HasAncestorNamed(instance, "colorpicker") or HasAncestorNamed(instance, "color picker")

		if instance:IsA("TextLabel") or instance:IsA("TextButton") or instance:IsA("TextBox") then
			instance.TextColor3 = DarkTheme.Text
			if instance:IsA("TextBox") then
				instance.PlaceholderColor3 = DarkTheme.Placeholder
			end
		end

		if instance:IsA("UIStroke") and not inColorPicker then
			instance.Color = DarkTheme.Outline
		end

		if instance:IsA("UIGradient") and string.lower(instance.Name) == "color" and not inColorPicker then
			if HasAncestorNamed(instance, "toggle") then
				instance.Color = SolidSequence(DarkTheme.Toggle)
			elseif HasAncestorNamed(instance, "slider") then
				instance.Color = SolidSequence(DarkTheme.Slider)
			else
				instance.Color = SolidSequence(DarkTheme.Primary)
			end
		end

		if not inColorPicker then
			if instance:IsA("Frame") or instance:IsA("ScrollingFrame") or instance:IsA("TextButton") or instance:IsA("TextBox") then
				if instance.BackgroundTransparency < 1 then
					if string.find(name, "label", 1, true) then
						instance.BackgroundColor3 = DarkTheme.LabelBackground
						if instance.BackgroundTransparency < DarkTheme.LabelBackgroundTransparency then
							instance.BackgroundTransparency = DarkTheme.LabelBackgroundTransparency
						end
					elseif string.find(name, "button", 1, true) or string.find(name, "action", 1, true) then
						instance.BackgroundColor3 = DarkTheme.Button
					elseif string.find(name, "dialog", 1, true) or string.find(name, "keysystem", 1, true) or string.find(name, "loading", 1, true) then
						instance.BackgroundColor3 = DarkTheme.Dialog
					else
						instance.BackgroundColor3 = DarkTheme.ElementBackground
					end
				end
			end

			if (instance:IsA("ImageLabel") or instance:IsA("ImageButton"))
				and not string.find(name, "shadow", 1, true)
				and not string.find(name, "logo", 1, true)
				and not HasAncestorNamed(instance, "player") then
				instance.ImageColor3 = DarkTheme.Icon
			end
		end
	end

	-- Explicit shell colors keep the visual hierarchy crisp.
	Main.BackgroundColor3 = DarkTheme.Background
	if Main:FindFirstChild("Elements") and Main.Elements:IsA("GuiObject") then Main.Elements.BackgroundColor3 = DarkTheme.Accent end
	if Navigation:IsA("GuiObject") then Navigation.BackgroundColor3 = DarkTheme.Accent end
	if KeySystem:IsA("GuiObject") then KeySystem.BackgroundColor3 = DarkTheme.Dialog end
	if LoadingFrame:IsA("GuiObject") then LoadingFrame.BackgroundColor3 = DarkTheme.Background end
	if Main:FindFirstChild("Line") then Main.Line.BackgroundColor3 = DarkTheme.Outline end
	if Navigation:FindFirstChild("Line") then Navigation.Line.BackgroundColor3 = DarkTheme.Outline end
end

-- Subtle micro-interactions for clickable cards/tabs. Sliders, color pickers and drag handles are intentionally excluded.
local MotionBound = setmetatable({}, {__mode = "k"})

local function BindMotion(interact)
	if not interact or not interact:IsA("GuiButton") or MotionBound[interact] then return end
	local target = interact.Parent
	if not target or not target:IsA("GuiObject") then return end

	local lname = string.lower(target.Name)
	if string.find(lname, "slider", 1, true)
		or string.find(lname, "colorpicker", 1, true)
		or string.find(lname, "color picker", 1, true)
		or string.find(lname, "drag", 1, true) then
		return
	end

	MotionBound[interact] = true
	local scale = target:FindFirstChild("LunaMotionScale")
	if not scale then
		scale = Instance.new("UIScale")
		scale.Name = "LunaMotionScale"
		scale.Scale = 1
		scale.Parent = target
	end
	...
	local hovered = false
	local pressed = false
	local function setScale(value, info)
		TweenService:Create(scale, info or Animation.Hover, {Scale = value}):Play()
	end

	target.MouseEnter:Connect(function()
		hovered = true
		if not pressed then setScale(1.012, Animation.Hover) end
	end)

	target.MouseLeave:Connect(function()
		hovered = false
		if not pressed then setScale(1, Animation.Hover) end
	end)

	interact.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			pressed = true
			setScale(0.992, Animation.Fast)
		end
	end)
	...
	interact.InputEnded:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			pressed = false
			setScale(hovered and 1.012 or 1, Animation.Hover)
		end
	end)
end

local function ScanForMotion(root)
	for _, instance in ipairs(root:GetDescendants()) do
		if instance.Name == "Interact" and instance:IsA("GuiButton") then
			BindMotion(instance)
		end
	end
end

ApplyDarkTheme(LunaUI)
ScanForMotion(LunaUI)
LunaUI.DescendantAdded:Connect(function(instance)
	if instance.Name == "Interact" and instance:IsA("GuiButton") then
		task.defer(BindMotion, instance)
	end
end)

Luna.ApplyTheme = function()
	ApplyDarkTheme(LunaUI)
end
	...
	pcall(function()
			...
			if dragBar and enableTaptic then
				dragBar.MouseEnter:Connect(function()
					if not Dragging then
						TweenService:Create(dragBarCosmetic, Animation.Hover, {BackgroundTransparency = 0.5, Size = UDim2.new(0, 120, 0, 4)}):Play()
					end
				end)

				dragBar.MouseLeave:Connect(function()
					if not Dragging then
						TweenService:Create(dragBarCosmetic, Animation.Hover, {BackgroundTransparency = 0.7, Size = UDim2.new(0, 100, 0, 4)}):Play()
					end
				end)
			end
		end
		...
		Bar.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				if enableTaptic then
					TweenService:Create(dragBarCosmetic, Animation.Normal, {Size = UDim2.new(0, 110, 0, 4), BackgroundTransparency = 0}):Play()
				end
				...
				Input.Changed:Connect(function()
					if Input.UserInputState == Enum.UserInputState.End then
						...
						if enableTaptic then
							TweenService:Create(dragBarCosmetic, Animation.Normal, {Size = UDim2.new(0, 100, 0, 4), BackgroundTransparency = 0.7}):Play()
						end
					end
				end)
			end
		...
		UserInputService.InputChanged:Connect(function(Input)
			if Input == DragInput and Dragging then
				...
				TweenService:Create(Window, Animation.Normal, {Position = newMainPosition}):Play()
...
function Luna:Notification(data) -- action e.g open messages
	task.spawn(function()
		...
		newNotification.Size = UDim2.new(1, 0, 0, -Notifications:FindFirstChild("UIListLayout").Padding.Offset)
		TweenService:Create(newNotification, Animation.Dialog, {Size = UDim2.new(1, 0, 0, bounds)}):Play()

		task.wait(0.15)
		TweenService:Create(newNotification, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.45}):Play()
		TweenService:Create(newNotification.Title, Animation.Normal, {TextTransparency = 0}):Play()

		task.wait(0.05)

		TweenService:Create(newNotification.Icon, Animation.Normal, {ImageTransparency = 0}):Play()

		task.wait(0.05)
		TweenService:Create(newNotification.Description, Animation.Normal, {TextTransparency = 0.35}):Play()
		TweenService:Create(newNotification.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Transparency = 0.95}):Play()
		TweenService:Create(newNotification.Shadow, Animation.Normal, {ImageTransparency = 0.82}):Play()
		...
		TweenService:Create(newNotification.Shadow, Animation.Normal, {ImageTransparency = 1}):Play()
		TweenService:Create(newNotification.Title, Animation.Normal, {TextTransparency = 1}):Play()
		TweenService:Create(newNotification.Description, Animation.Normal, {TextTransparency = 1}):Play()
	...
	for _, tabbtn in ipairs(Window.Navigation.Tabs:GetChildren()) do
		if tabbtn.ClassName == "Frame" and tabbtn.Name ~= "InActive Template" then
			if tabbtn.Name == currentTab then
				TweenService:Create(tabbtn, Animation.Normal, {BackgroundTransparency = 0}):Play()
				TweenService:Create(tabbtn.UIStroke, Animation.Normal, {Transparency = 0.41}):Play()
			end
			TweenService:Create(tabbtn.ImageLabel, Animation.Normal, {ImageTransparency = 0}):Play()
			TweenService:Create(tabbtn.DropShadowHolder.DropShadow, Animation.Normal, {ImageTransparency = 1}):Play()
		end
	end
...
function Luna:CreateWindow(WindowSettings)
		...
		if not Passthrough then
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					for _, instance in pairs(KeySystem:GetDescendants()) do
						if instance.ClassName ~= "UICorner" and instance.ClassName ~= "UIPadding" then
							if instance.ClassName ~= "UIStroke" and instance.ClassName ~= "UIListLayout" then
								tween(instance, {BackgroundTransparency = 1}, nil,Animation.Dialog)
							end
					...
					tween(KeySystem, {BackgroundTransparency = 1}, nil,Animation.Dialog)
					task.wait(0.51)
					Passthrough = true
	...
	if WindowSettings.LoadingEnabled then
		task.wait(0.3)
		TweenService:Create(LoadingFrame.Frame.Frame.Title, Animation.Reveal, {TextTransparency = 0}):Play()
		TweenService:Create(LoadingFrame.Frame.ImageLabel, Animation.Normal, {ImageTransparency = 0}):Play()
		task.wait(0.05)
		TweenService:Create(LoadingFrame.Frame.Frame.Subtitle, Animation.Reveal, {TextTransparency = 0}):Play()
		TweenService:Create(LoadingFrame.Version, Animation.Reveal, {TextTransparency = 0}):Play()
		task.wait(0.29)
		...
		TweenService:Create(LoadingFrame.Frame.Frame.Title, Animation.Reveal, {TextTransparency = 1}):Play()
		TweenService:Create(LoadingFrame.Frame.ImageLabel, Animation.Normal, {ImageTransparency = 1}):Play()
		task.wait(0.05)
		TweenService:Create(LoadingFrame.Frame.Frame.Subtitle, Animation.Reveal, {TextTransparency = 1}):Play()
		TweenService:Create(LoadingFrame.Version, Animation.Reveal, {TextTransparency = 1}):Play()
		wait(0.3)
		TweenService:Create(LoadingFrame, Animation.Window, {BackgroundTransparency = 1}):Play()
	end

	TweenService:Create(Main, Animation.Window, {BackgroundTransparency = 0.2, Size = MainSize}):Play()
	TweenService:Create(Main.Parent.ShadowHolder, Animation.Window, {Size = MainSize}):Play()
	TweenService:Create(Main.Title.Title, Animation.Normal, {TextTransparency = 0}):Play()
	TweenService:Create(Main.Title.subtitle, Animation.Normal, {TextTransparency = 0}):Play()
	TweenService:Create(Main.Logo, Animation.Normal, {ImageTransparency = 0}):Play()
	TweenService:Create(Navigation.Player.icon.ImageLabel, Animation.Normal, {ImageTransparency = 0}):Play()
	TweenService:Create(Navigation.Player.icon.UIStroke, Animation.Normal, {Transparency = 0}):Play()
	TweenService:Create(Main.Line, Animation.Normal, {BackgroundTransparency = 0}):Play()
	wait(0.4)
		...
		function HomeTab:Activate()
			...
			for _, OtherTabButton in ipairs(Navigation.Tabs:GetChildren()) do
				if OtherTabButton.Name ~= "InActive Template" and OtherTabButton.ClassName == "Frame" and OtherTabButton ~= HomeTabButton then
					tween(OtherTabButton.ImageLabel, {ImageColor3 = DarkTheme.Icon})
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:Activate()
			tween(TabButton.ImageLabel, {ImageColor3 = DarkTheme.Text})
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateButton(ButtonSettings)
				...
				TweenService:Create(Button, Animation.Reveal, {BackgroundTransparency = 0.5}):Play()
				TweenService:Create(Button.UIStroke, Animation.Reveal, {Transparency = 0.5}):Play()
				TweenService:Create(Button.Title, Animation.Reveal, {TextTransparency = 0}):Play()	
				if ButtonSettings.Description ~= nil and ButtonSettings.Description ~= "" then
					TweenService:Create(Button.Desc, Animation.Reveal, {TextTransparency = 0}):Play()	
				end
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						TweenService:Create(Button, Animation.Reveal, {BackgroundTransparency = 0}):Play()
						TweenService:Create(Button, Animation.Reveal, {BackgroundColor3 = Color3.fromRGB(85, 0, 0)}):Play()
						TweenService:Create(Button.UIStroke, Animation.Reveal, {Transparency = 1}):Play()
						Button.Title.Text = "Callback Error"
						print("Luna Interface Suite | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(Button, Animation.Reveal, {BackgroundTransparency = 0.5}):Play()
						TweenService:Create(Button, Animation.Reveal, {BackgroundColor3 = DarkTheme.Button}):Play()
						TweenService:Create(Button.UIStroke, Animation.Reveal, {Transparency = 0.5}):Play()
					else
						tween(Button.UIStroke, {Color = DarkTheme.Primary})
						wait(0.2)
						if ButtonV.Hover then
							tween(Button.UIStroke, {Color = DarkTheme.Primary})
						else
							tween(Button.UIStroke, {Color = DarkTheme.Icon})
						end
					end
				end)
			...
			function Section:CreateLabel(LabelSettings)
				...
				if LabelSettings.Style ~= 1 then
					TweenService:Create(Label, Animation.Reveal, {BackgroundTransparency = 0.8}):Play()
				else
					TweenService:Create(Label, Animation.Reveal, {BackgroundTransparency = 1}):Play()
				end
				TweenService:Create(Label.UIStroke, Animation.Reveal, {Transparency = 0.5}):Play()
				TweenService:Create(Label.Text, Animation.Reveal, {TextTransparency = 0}):Play()	
			...
			function Section:CreateParagraph(ParagraphSettings)
				...
				TweenService:Create(Paragraph, Animation.Reveal, {BackgroundTransparency = 1}):Play()
				TweenService:Create(Paragraph.UIStroke, Animation.Reveal, {Transparency = 0.5}):Play()
				TweenService:Create(Paragraph.Title, Animation.Reveal, {TextTransparency = 0}):Play()	
				TweenService:Create(Paragraph.Text, Animation.Reveal, {TextTransparency = 0}):Play()	
			...
			function Section:CreateSlider(SliderSettings, Flag)
				...
				Slider.Main.color.Color = SolidSequence(DarkTheme.Slider)
				Slider.Main.UIStroke.color.Color = SolidSequence(DarkTheme.Slider)
				Slider.Visible = true
				...
				TweenService:Create(Slider, Animation.Reveal, {BackgroundTransparency = 0.5}):Play()
				TweenService:Create(Slider.UIStroke, Animation.Reveal, {Transparency = 0.5}):Play()
				TweenService:Create(Slider.Title, Animation.Reveal, {TextTransparency = 0}):Play()	
				...
				Slider["MouseEnter"]:Connect(function()
					tween(Slider.UIStroke, {Color = DarkTheme.Primary})
				end)

				Slider["MouseLeave"]:Connect(function()
					tween(Slider.UIStroke, {Color = DarkTheme.Icon})
				end)
					...
					local Loop; Loop = RunService.Stepped:Connect(function()
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									TweenService:Create(Slider, Animation.Reveal, {BackgroundTransparency = 0}):Play()
									TweenService:Create(Slider, Animation.Reveal, {BackgroundColor3 = Color3.fromRGB(85, 0, 0)}):Play()
									TweenService:Create(Slider.UIStroke, Animation.Reveal, {Transparency = 1}):Play()
									Slider.Title.Text = "Callback Error"
									print("Luna Interface Suite | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, Animation.Reveal, {BackgroundTransparency = 0.5}):Play()
									TweenService:Create(Slider, Animation.Reveal, {BackgroundColor3 = DarkTheme.ElementBackground}):Play()
									TweenService:Create(Slider.UIStroke, Animation.Reveal, {Transparency = 0.5}):Play()
								end
							...
							TweenService:Create(Slider.Main.Progress, Animation.Slider, {Size = UDim2.new(0, Location - Slider.Main.AbsolutePosition.X > 5 and Location - Slider.Main.AbsolutePosition.X or 5, 1, 0)}):Play()
							Loop:Disconnect()
						end
					end)
				end)
				...
				local function Set(NewVal, bleh)
					...
					TweenService:Create(Slider.Main.Progress, Animation.SliderSet, {Size = UDim2.new(0, Slider.Main.AbsoluteSize.X * ((NewVal + SliderSettings.Range[1]) / (SliderSettings.Range[2] - SliderSettings.Range[1])) > 5 and Slider.Main.AbsoluteSize.X * (NewVal / (SliderSettings.Range[2] - SliderSettings.Range[1])) or 5, 1, 0)}):Play()
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				Toggle.toggle.color.Color = SolidSequence(DarkTheme.Toggle)
				Toggle.toggle.UIStroke.color.Color = SolidSequence(DarkTheme.Toggle)
				...
				TweenService:Create(Toggle, Animation.Reveal, {BackgroundTransparency = 0.5}):Play()
				if ToggleSettings.Description ~= nil and ToggleSettings.Description ~= "" then
					TweenService:Create(Toggle.Desc, Animation.Reveal, {TextTransparency = 0}):Play()
				end
				TweenService:Create(Toggle.UIStroke, Animation.Reveal, {Transparency = 0.5}):Play()
				TweenService:Create(Toggle.Title, Animation.Reveal, {TextTransparency = 0}):Play()	
				...
				local function Set(bool)
					if bool then
						...
						tween(Toggle.toggle.UIStroke, {Color = DarkTheme.Outline})
						...
						Toggle.toggle.UIStroke.Color = DarkTheme.SliderIcon

						tween(Toggle.toggle, {BackgroundTransparency = 1})

						tween(Toggle.toggle.val, {BackgroundColor3 = DarkTheme.SliderIcon, Position = UDim2.new(0,5,0.5,0), BackgroundTransparency = 0})
					end
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						TweenService:Create(Toggle, Animation.Reveal, {BackgroundTransparency = 0}):Play()
						TweenService:Create(Toggle, Animation.Reveal, {BackgroundColor3 = Color3.fromRGB(85, 0, 0)}):Play()
						TweenService:Create(Toggle.UIStroke, Animation.Reveal, {Transparency = 1}):Play()
						Toggle.Title.Text = "Callback Error"
						print("Luna Interface Suite | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, Animation.Reveal, {BackgroundTransparency = 0.5}):Play()
						TweenService:Create(Toggle, Animation.Reveal, {BackgroundColor3 = DarkTheme.ElementBackground}):Play()
						TweenService:Create(Toggle.UIStroke, Animation.Reveal, {Transparency = 0.5}):Play()
					end
				end)

				Toggle["MouseEnter"]:Connect(function()
					tween(Toggle.UIStroke, {Color = DarkTheme.Primary})
				end)

				Toggle["MouseLeave"]:Connect(function()
					tween(Toggle.UIStroke, {Color = DarkTheme.Icon})
				end)
				...
				function ToggleV:Set(NewToggleSettings)
					...
					if not Success then
						TweenService:Create(Toggle, Animation.Reveal, {BackgroundTransparency = 0}):Play()
						TweenService:Create(Toggle, Animation.Reveal, {BackgroundColor3 = Color3.fromRGB(85, 0, 0)}):Play()
						TweenService:Create(Toggle.UIStroke, Animation.Reveal, {Transparency = 0}):Play()
						Toggle.Title.Text = "Callback Error"
						print("Luna Interface Suite | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
			...
			function Section:CreateBind(BindSettings, Flag)
				...
				TweenService:Create(Bind, Animation.Normal, {BackgroundTransparency = 0.5}):Play()
				TweenService:Create(Bind.Title, Animation.Normal, {TextTransparency = 0}):Play()
				if BindSettings.Description ~= nil and BindSettings.Description ~= "" then
					TweenService:Create(Bind.Desc, Animation.Normal, {TextTransparency = 0}):Play()
				end
				TweenService:Create(Bind.BindFrame, Animation.Normal, {BackgroundTransparency = 0.9}):Play()
				TweenService:Create(Bind.BindFrame.UIStroke, Animation.Normal, {Transparency = 0.3}):Play()
				TweenService:Create(Bind.BindFrame.BindBox, Animation.Normal, {TextTransparency = 0}):Play()
				...
				Bind["MouseEnter"]:Connect(function()
					tween(Bind.UIStroke, {Color = DarkTheme.Primary})
				end)

				Bind["MouseLeave"]:Connect(function()
					tween(Bind.UIStroke, {Color = DarkTheme.Icon})
				end)
				UserInputService.InputBegan:Connect(function(input, processed)
					...
					if CheckingForKey then
						if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
							...
							if not Success then
								TweenService:Create(Bind, Animation.Reveal, {BackgroundTransparency = 0}):Play()
								TweenService:Create(Bind, Animation.Reveal, {BackgroundColor3 = Color3.fromRGB(85, 0, 0)}):Play()
								TweenService:Create(Bind.UIStroke, Animation.Reveal, {Transparency = 1}):Play()
								Bind.Title.Text = "Callback Error"
								print("Luna Interface Suite | "..BindSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Bind.Title.Text = BindSettings.Name
								TweenService:Create(Bind, Animation.Reveal, {BackgroundTransparency = 0.5}):Play()
								TweenService:Create(Bind, Animation.Reveal, {BackgroundColor3 = DarkTheme.ElementBackground}):Play()
								TweenService:Create(Bind.UIStroke, Animation.Reveal, {Transparency = 0.5}):Play()
							end
			...
			function Section:CreateInput(InputSettings, Flag)
				...
				TweenService:Create(Input, Animation.Normal, {BackgroundTransparency = 0.5}):Play()
				TweenService:Create(Input.UIStroke, Animation.Normal, {Transparency = 0.5}):Play()
				TweenService:Create(Input.Title, Animation.Normal, {TextTransparency = 0}):Play()	
				if descriptionbool then TweenService:Create(Input.Desc, Animation.Normal, {TextTransparency = 0}):Play() end
				TweenService:Create(Input.InputFrame, Animation.Normal, {BackgroundTransparency = 0.9}):Play()
				TweenService:Create(Input.InputFrame.UIStroke, Animation.Normal, {Transparency = 0.3}):Play()
				TweenService:Create(Input.InputFrame.InputBox, Animation.Normal, {TextTransparency = 0}):Play()
				...
				Input.InputFrame.InputBox.FocusLost:Connect(function(bleh)
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								TweenService:Create(Input, Animation.Reveal, {BackgroundTransparency = 0}):Play()
								TweenService:Create(Input, Animation.Reveal, {BackgroundColor3 = Color3.fromRGB(85, 0, 0)}):Play()
								TweenService:Create(Input.UIStroke, Animation.Reveal, {Transparency = 1}):Play()
								Input.Title.Text = "Callback Error"
								print("Luna Interface Suite | "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
								TweenService:Create(Input, Animation.Reveal, {BackgroundTransparency = 0.5}):Play()
								TweenService:Create(Input, Animation.Reveal, {BackgroundColor3 = DarkTheme.ElementBackground}):Play()
								TweenService:Create(Input.UIStroke, Animation.Reveal, {Transparency = 0.5}):Play()
							end
						end
					end
				...
				Input["MouseEnter"]:Connect(function()
					tween(Input.UIStroke, {Color = DarkTheme.Primary})
				end)

				Input["MouseLeave"]:Connect(function()
					tween(Input.UIStroke, {Color = DarkTheme.Icon})
				end)
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						TweenService:Create(Dropdown, Animation.Reveal, {BackgroundTransparency = 0}):Play()
						TweenService:Create(Dropdown, Animation.Reveal, {BackgroundColor3 = Color3.fromRGB(85, 0, 0)}):Play()
						TweenService:Create(Dropdown.UIStroke, Animation.Reveal, {Transparency = 1}):Play()
						Dropdown.Title.Text = "Callback Error"
						print("Luna Interface Suite | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, Animation.Reveal, {BackgroundTransparency = 0.5}):Play()
						TweenService:Create(Dropdown, Animation.Reveal, {BackgroundColor3 = DarkTheme.ElementBackground}):Play()
						TweenService:Create(Dropdown.UIStroke, Animation.Reveal, {Transparency = 0.5}):Play()
					end
				...
				Dropdown["MouseEnter"]:Connect(function()
					tween(Dropdown.UIStroke, {Color = DarkTheme.Primary})
				end)

				Dropdown["MouseLeave"]:Connect(function()
					tween(Dropdown.UIStroke, {Color = DarkTheme.Icon})
				end)
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				...
				ColorPicker["MouseEnter"]:Connect(function()
					tween(ColorPicker.UIStroke, {Color = DarkTheme.Primary})
				end)
				ColorPicker["MouseLeave"]:Connect(function()
					tween(ColorPicker.UIStroke, {Color = DarkTheme.Icon})
				end)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						TweenService:Create(ColorPicker, Animation.Reveal, {BackgroundTransparency = 0}):Play()
						TweenService:Create(ColorPicker, Animation.Reveal, {BackgroundColor3 = Color3.fromRGB(85, 0, 0)}):Play()
						TweenService:Create(ColorPicker.UIStroke, Animation.Reveal, {Transparency = 1}):Play()
						...
						wait(0.5)
						ColorPicker.Title.Text = ColorPickerSettings.Name
						TweenService:Create(ColorPicker, Animation.Reveal, {BackgroundTransparency = 0.5}):Play()
						TweenService:Create(ColorPicker, Animation.Reveal, {BackgroundColor3 = DarkTheme.ElementBackground}):Play()
						TweenService:Create(ColorPicker.UIStroke, Animation.Reveal, {Transparency = 0.5}):Play()
					end
					if Success and c2 then
						c2()
					end
				...
				ColorPicker.Interact.MouseButton1Down:Connect(function()
					if not opened then
						opened = true 
						tween(ColorPicker, {Size = UDim2.new( 1.042, -25,0, 165)}, nil, Animation.Dialog)
						tween(Background, {Size = openedsize})
						tween(Display, {BackgroundTransparency = 1})
					else
						opened = false
						tween(ColorPicker, {Size = UDim2.new(1.042, -25,0, 38)}, nil, Animation.Dialog)
